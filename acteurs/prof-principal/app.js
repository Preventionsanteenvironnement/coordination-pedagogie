/* Professeur principal — multi-pages mobile. Sources cliquables. */
let page = "home";   // home | section index
let idx = 0;
const $ = s => document.querySelector(s);
const esc = v => String(v ?? "").replaceAll("&", "&amp;").replaceAll('"', "&quot;").replaceAll("<", "&lt;");
const TITRE = (window.SITE && window.SITE.titre) || "Le professeur principal";
...
function viewHome() {
  const lead = (window.SITE && window.SITE.lead) || { strong: TITRE, span: "" };
  return `
    <div class="lead"><strong>${esc(lead.strong)}</strong><span>${esc(lead.span)}</span></div>
