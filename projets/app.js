...
function synthBoard(e){
  ...
  const directBtn = co?`<button class="ev-direct ${projet.directMode?"on":""}" data-directmode title="${projet.directMode?"Mode direct ACTIVÉ pour TOUT le projet : les propositions des collègues entrent directement dans le tableau, à chaque étape. Cliquez pour reprendre la main (validation par le pilote).":"Activer le mode direct pour TOUT le projet : les propositions des collègues iront droit dans le tableau, à chaque étape, sans validation de votre part."}">${ic("bolt")} Mode direct · ${projet.directMode?"ON":"OFF"}</button>`:"";
  const directNote = projet.directMode?`<div class="ev-direct-note">${ic("bolt")} Mode direct activé pour tout le projet — chaque nouvelle proposition arrive directement dans le tableau.</div>`:"";
