    ...
    const blocHtml = (c, large) => {
      ...
      if (large) {
        ...
        return `<div class="bloc ${aeshs.length ? 'avec-presences' : 'sans-presence'} avec-besoin ${S.flash === c.id ? 'flash' : ''}" style="--mc:${mc};top:${top + 1}px;height:${h - 2}px;left:${3 + (c._col || 0) * (dispo / nbc)}%;width:calc(${dispo / nbc}% - 6px)${lieu ? '' : ';opacity:.45'}">
          <button type="button" class="bloc-contenu ${lettre ? 'une-sem' : ''}" id="c-${esc(c.id)}" data-a="${lectureSeule ? 'lecture' : 'placer'}" data-v="${esc(c.id)}" aria-label="${lettre ? 'Semaine ' + lettre + '. ' : ''}${esc(lib)}"><b>${lettre ? `<i class="sem-lettre ${lettre === 'B' ? 'b' : ''}">${lettre}</i>` : ''}${esc(c.lib)}</b>${h > 116 && sallesCourtes(c.salle) ? `<span class="salle">${esc(sallesCourtes(c.salle))}</span>` : ''}${h > 96 ? elvHtml : ''}<span class="past-ligne" aria-hidden="true">${besHtml}${bandes}</span><span class="sr">${aeshs.map(a=>esc(a.sigle)).join(", ")}</span></button></div>`;
      }
